...
await Promise.all([
  ...
  writeOutput('toolsafe-report.json', renderJsonReport(result)),
  writeOutput('TOOLSAFE_REPORT.md', renderMarkdownReport(result)),
  writeOutput('guard-policy.yaml', renderPolicyYaml(generatePolicyDraft(result))),
  writeOutput('toolsafe.evals.yaml', renderEvalIdeasYaml(generateEvalIdeas(result))),
]);
